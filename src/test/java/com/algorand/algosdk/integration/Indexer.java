...
public class Indexer {
    ...
    @Then("There are {int} with the asset, the first is {string} has {string} and {long}")
    public void there_are_with_the_asset_the_first_is_has_and(Integer numResults, String account, String frozenState, Long amount) {
    ...
    public void the_first_account_is_online_and_has(String address, Long keyDilution, Long firstValid, Long lastValid, String voteKey, String selectionKey) {
        ...
        assertThat(participation.voteParticipationKey).isEqualTo(voteKey);
        assertThat(participation.selectionParticipationKey).isEqualTo(selectionKey);
    }
    ...
    @And("Every transaction moves between {long} and {long} currency")
    public void every_transaction_moves_between_and_currency(Long min, Long max) {
        // Normalize in case max isn't set.
        final Long normalizedMax = (max == 0) ? Long.MAX_VALUE : max;

        transactionsResponse.body().transactions.forEach(tx -> {
            switch (tx.type) {
                case PAY:
                    assertThat(tx.paymentTransaction.amount).isBetween(min, normalizedMax);
                    break;
                case AXFER:
